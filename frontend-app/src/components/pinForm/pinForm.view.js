...
const PinForm = () => {
    ...
    const submitForm = () => {
        ...
        const myToken = getToken(); // NOTE - Auth Create Pin
        const options = {
            method: 'POST',
            headers: new Headers({
                'Content-type': 'application/json',
                Accept: 'application/json', // NOTE - Auth Create Pin
                authorization: `Bearer ${myToken.token}`, // NOTE - Auth Create Pin
            }),
            mode: 'cors',
        ...
        fetch(url, options)
            ...
            .then((payload) => {
                setSuccess('Your pin has been created'); // NOTE - Auth Create Pin
                console.log('New pin added to DB =>', payload);
            })
            ...
            <div className={styles.__container}>
                ...
                <Form>
                    ...
                    <Form.Group controlId="formGridPinMediaUrl">
                        ...
                        <Form.Control
                            placeholder="Your Media Url Here"
                    ...
                    <Form.Group controlId="formGridPinBoardName">
                        ...
                        <Form.Control
                            as="select"
                            defaultValue="Choose a board name..."
                        ...
                        >
                            ...
                            */}
                            ...
                            {boards.map((board) => {
                                return (
                                    <option key={`board-select${board.id}`} value={board.id}>
                                        {' '}
                                        {board.name}{' '}
                                    </option>
                                );
                    ...
                    <Button
                        className={cx('btn btn-dark', styles.__form__button__item)}
                        variant="dark"
                        type="submit"
                        onClick={submitForm}
                    >
